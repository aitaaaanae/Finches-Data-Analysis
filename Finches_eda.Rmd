...
library(Amelia)
# Save the missmap as a PNG file
png("missmap.png", width = 800, height = 800)

# Create a missmap object from the table
...
#tbl_num <- tbl_summary(new_tbl_numeric, everything ~ "mean_sd")
#print(tbl_num)
...
```{r Plot distributions of cat and numeric variables, echo=FALSE}



finches_table$Sex <- fct_collapse(
  as.factor(finches_table$Sex),
  male = "male",
  female = "female",
  unknown = "unknown"
)
...
```{r cont dist29, fig.width=7, fig.height=7, out.width='600px' }
...
2. Correlation between morphological variables: 
...
# The plot is automatically saved when you close the device
```
...
###For the first hypothesis, we seek to study the relationship between survival status and different morphological characteristics related to the beak: 
...
finches_table_numeric %>%
  select("Beak.Width..mm.", "Beak.Depth..mm.", "Beak.Length..mm.", "Beak.Size", "survivor") %>%
  tbl_summary(
    by = survivor,
    statistic = list(
      "Beak.Width..mm." ~ "{mean} ({sd})",
...
## 2. There is sexual dimorphism after the drought: 
...
# Create a PDF device to save the plots
pdf("density_box_plots_sex.pdf", width = 10, height = 8)

# Define custom color palette
my_colors <- c("male" = "steelblue", "female" = "salmon", "unknown" = "gray")

for (var in continuous_vars) {
  # Create density plot
  density_plot <- ggplot(finches_table_numeric, aes(x = .data[[var]], fill = Sex)) +
    geom_density(alpha = 0.6) +
    labs(title = paste("Density Plot of", var, "vs. Sex")) +
```
...
scale_fill_manual(values = my_colors) +
  ...
  # Create box plot
  box_plot <- ggplot(finches_table_numeric, aes(x = Sex, y = .data[[var]], fill = Sex)) +
    geom_boxplot() +
    labs(title = paste("Box Plot of", var, "vs. Sex")) +
    scale_fill_manual(values = my_colors) +
    theme_minimal() +
...
#Extra hypotheses: not included in the report--> age of the finches at the moment of the drought affected their survival?
```{r exper8, fig.width=3, fig.height=3, out.width='300px}
